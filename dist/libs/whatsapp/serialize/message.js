...
const message = async (aruga, msg) => {
    ...
    if (m.message) {
        ...
        function reply(text, quoted = false) {
            return aruga.sendMessage(m.from, { text, ...(m.isGroupMsg ? { mentions: [m.sender] } : {}) }, {
                ...(quoted ? { quoted: m } : {}),
    ...
    m.groupMetadata = m.isGroupMsg && ((await whatsapp_1.database.getGroupMetadata(m.from)) ?? (await whatsapp_1.database.createGroupMetadata(m.from, (await aruga.groupMetadata(m.from)))));
    m.quoted = {};
    ...
    if (m.quoted.message) {
        ...
        function reply(text, quoted = false) {
            return aruga.sendMessage(m.from, { text, ...(m.quoted.isGroupMsg ? { mentions: [m.quoted.sender] } : {}) }, {
